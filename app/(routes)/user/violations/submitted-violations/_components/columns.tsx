...
export const columns: ColumnDef<SubmittedViolationsColumn>[] = [
        ...
        <Badge
          className={cn(
            ...
            row.getValue('status') === 'For Review'
              ...
              ? 'bg-green-700'
              : row.getValue('status') === 'Closed' &&
                row.original.reasonToClose === 'Insufficient Evidence'
              ? 'bg-red-800'
              : ''
          )}
        >
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <Text textAlign='center'>
        #V{row.original.number.toString().padStart(4, '0')}
      </Text>
    )
  },
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div>
        <a
          href={`/user/violations/submitted-violations/view-progress/${row.original.id}`}
