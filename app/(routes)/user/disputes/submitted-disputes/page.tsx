...
export default async function SubmittedDisputes() {
  ...
  const filteredDisputes = await disputes.filter(
    (dispute) => dispute.submittedBy === user.id
  );

  let formattedDisputes: SubmittedDisputesColumn[] = filteredDisputes.map(
    (item) => {
      const officer = users.find(
        (user) => user.userId === item.officerAssigned
      );
      const submittedBy = users.find(
        (user) => user.userId === item.submittedBy
      );
  ...
  return (
    <div>
      <div className="flex-1 space-y-4">
        <SubmittedDisputesClient data={formattedDisputes} hoaInfo={hoaInfo} />
      </div>
    </div>
  );
}
