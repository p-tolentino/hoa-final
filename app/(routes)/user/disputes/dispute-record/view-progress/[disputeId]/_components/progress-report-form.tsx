...
export default function ProgressReportForm ({
  ...
  const onSubmit = async (isSubActivity: boolean) => {
    ...
    await createDisputeProgressReport(formData).then(data => {
      console.log(data.success)
    })

    if (!isSubActivity) {
      await updateDisputeOfficerTask(forActivity, true).then(data => {
        ...
        if (allDoneExceptLast) {
          const update = {
            step: 6,
            progress: 'Step 6: Dispute Resolution with Corrective Actions',
