...
export type MaintenanceRecordColumn = {
  id: string
  number: number
  status: string
  createdAt: string
...
export const columns: ColumnDef<MaintenanceRecordColumn>[] = [
  {
    accessorKey: 'status',
    header: ({ column }) => {
      return (
    ...
    cell: ({ row }) => (
      <>
        <Badge
          className={cn(
            'w-[200px] md:text-xs px-3 py-2 text-center justify-center break-text',
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div>
        <a
          href={`/user/maintenance/maintenance-record/view-progress/${row.original.id}`}
